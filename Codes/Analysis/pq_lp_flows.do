...
forvalues block = 1/4 {
	local ++i
	if `i' == 1 {
		local ctg "Instit1"							// "Instit"
		local vars banks mutual pension 			// insurers
	}
	if `i' == 2 {
		local ctg "Instit2"							// "Other"
		local vars insurers others foreigners 		// repos collateral
	}
	if `i' == 3 {
		local ctg "Instit3"						// "Total"
		local vars domestic foreigners total
	}
	if `i' == 4 {
		local ctg "Instit4"						// "Repos"
		local vars repos collateral banxico
	}
	...
	// 				local condition em == `group'	// & `region' == 4
			...
			foreach shock in `indvars' {
				...
				foreach var in `vars' {
					local v `scty'`var'
					local ++k
					if `k' == 1 local yxtitles ytitle("MXN   Billions", size(medsmall)) xtitle("Days", size(medsmall))
	...
	// 				graph export $pathfigs/LPs/`shk'/`grp'/`v'`t'.eps, replace
				...
				graph export $pathfigs/LPs/`shk'/`grp'/`shk'`t'`grp'`ctg'.eps, replace
				graph drop _all
